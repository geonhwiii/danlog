...
```md
2012 webpack 등장 - 최초의 모던 번들러, CommonJS 중심 - 강력하지만 복잡한 설정, 느린 HMR

2015 rollup 등장 - ES 모듈 우선 설계 - 더 뛰어난 트리셰이킹, 라이브러리용 특화 - vite, tsdown 플러그인 API의 원형

2020 esbuild 등장 - Go 기반, webpack 대비 10~100배 빠름 - .d.ts 미지원, 라이브러리 제작 관습 부재
-> 부족한 부분을 채우기 위해 tsup(2021) 등장

2020 vite 등장 - 개발 서버: esbuild + 네이티브 ESM (번들링 없음) - 프로덕션 빌드: rollup 사용 - 문제점: 두 파이프라인 → dev/prod 불일치

2023 rolldown 등장 - Rust 기반 rollup 대체 번들러 - esbuild + rollup 역할 통합 - Vite 8 코어 번들러로 채택
